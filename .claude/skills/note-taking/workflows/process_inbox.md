...
3. **Origin:** How was this note created?
   - `braindump` - Captured via braindump workflow
   - `ai-output` - Generated by a Domain Agent
   - `manual` - User-created content (default)

   **If user selects `ai-output`:** Prompt for `origin_agent` — "Which agent generated this content?"
...
|-----------------|----------------|
| Contains meeting attendees, action items | `meeting` |
| Early-stage concept, not fully developed | `idea` |
| External resource, URL, documentation | `reference` |
| Contains URL/source | `reference` |
| Contains "decided", options considered | `decision` |
| Explains/defines a term | `concept` |
| Core belief or value statement | `belief` |
| Mental perspective or lens | `frame` |
| Lesson or realization | `lesson` |
| Decision framework or heuristic | `model` |
| Objective or aspiration | `goal` |
...
Available types:
 ...
 7. belief    - Core belief or value
 8. frame     - Mental perspective or lens
 9. lesson    - Lesson or realization
10. model     - Decision framework or heuristic
11. goal      - Objective or aspiration
12. plan      - Plan or roadmap
...
**Condition:** Only runs when origin: braindump
...
origin_agent: [agent-name if origin: ai-output, else null]
url: null
favorite: false
...
### Notes Processed
| Note | Domain | Project | Type | Origin |
|------|--------|---------|------|--------|
| note_name.md | ProjectAlpha | PROJECT_FEATURE_X.md | reference | manual |
| another_note.md | BlogContent | null | idea | braindump |
